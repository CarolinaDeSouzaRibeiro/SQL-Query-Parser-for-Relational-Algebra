"""
Módulo para geração automatizada de arquivos SQL com dados sintéticos para testes de desempenho.

Este script utiliza a biblioteca Faker para gerar dados fictícios realistas em português (Brasil),
e cria arquivos .sql com comandos INSERT correspondentes a diferentes configurações de volume de dados.
Cada configuração representa um cenário de teste com quantidades variadas de registros por tabela.

Os arquivos gerados são salvos no diretório `configuracoes/` e podem ser utilizados para popular um banco de dados SQLite
em testes de performance, validação de estruturas de dados, ou simulações de carga.

Principais funcionalidades:
- Criação de dados para tabelas como Cliente, Produto, Pedido, Endereço, entre outras.
- Suporte a múltiplas configurações de volume (ex: configuração1, configuração2...).
- Barra de progresso opcional para acompanhamento da geração.

Requisitos:
- Python 3.9+
- Bibliotecas: faker, tqdm
"""
...
def definir_configuracoes(ver_progresso: bool = True) -> None:
    ...
    os.makedirs(base_dir / "configuracoes", exist_ok=True)
